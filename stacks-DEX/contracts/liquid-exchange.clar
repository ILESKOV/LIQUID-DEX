...
(define-private (provide-liquidity-first (stx-amount uint) (token-amount uint) (provider principal))
    (begin
      ...
      (try! (contract-call? .liquid transfer token-amount tx-sender (as-contract tx-sender)))
      ...
      (as-contract (contract-call? .liquid-lp mint stx-amount provider))
    )
)
...
(define-private (provide-liquidity-additional (stx-amount uint))
  (let (
    ...
    (begin 
      ...
      (try! (contract-call? .liquid transfer tokens-to-transfer tx-sender contract-address))
      ;; mint LP tokens to tx-sender
      ;; inside as-contract the tx-sender is the exchange contract, so we use tx-sender passed into the function
      (as-contract (contract-call? .liquid-lp mint liquidity-to-mint provider))
    )
  )
)
  ...
  (begin 
    ...
    (let (
      ...
      (begin
        ...
        (as-contract (contract-call? .liquid transfer tokens-to-pay contract-address user-address))
      )
    )
  )
)
